...
function boot(seed=1234){
 ...
 const env={console,Math:math,Map,Set,document:{hidden:false,querySelector(s){if(!els.has(s))els.set(s,new Element());return els.get(s)},createElement:()=>new Element(),addEventListener(){}},localStorage:{getItem:()=>null,setItem(){}},requestAnimationFrame(){},addEventListener(){},setTimeout(){},devicePixelRatio:1};env.window=env;vm.createContext(env);
 vm.runInContext(fs.readFileSync(path.join(base,'physics.js'),'utf8'),env);
 const source=fs.readFileSync(path.join(base,'game.js'),'utf8').replace(/\}\)\(\);\s*$/,`window.test={get:()=>({run,claw,balls,phase,paused,delivered,totals}),update,drop,newRun,startFloor,pause,closeModal,setPhase,render,engine};})();`);
 vm.runInContext(source,env);return {test:env.test,els};
}
...
t.newRun();t.get().balls.length=0;assert.equal(playDrop(245),0,'empty grab completes without inventing cargo');assert.ok(t.get().run.hp<40,'empty grab gives enemy a turn');
