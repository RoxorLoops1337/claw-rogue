...
const { version, buildNumber } = require('../mobile/package.json');
if (!Number.isSafeInteger(buildNumber) || buildNumber < 1) throw new Error('A positive native buildNumber is required.');
...
if (platform === 'android') {
  ...
  edit('mobile/android/app/build.gradle', source => source.replace(/versionCode \d+/, `versionCode ${buildNumber}`).replace(/versionName "[^"]*"/, `versionName "${version}"`));
  ...
  edit('mobile/ios/App/App.xcodeproj/project.pbxproj', source => source.replace(/MARKETING_VERSION = [^;]+;/g, `MARKETING_VERSION = ${version};`).replace(/CURRENT_PROJECT_VERSION = [^;]+;/g, `CURRENT_PROJECT_VERSION = ${buildNumber};`));
